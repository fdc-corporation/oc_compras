...
class ServidorCorreos(models.Model):
        ...
        for record in self:
            try:
                ...
                if status == "OK":
                    ...
                    return {'status': 'success', 'message': 'Conexión establecida exitosamente.', 'mensajes': mensajes}
                else:
                    record.estado = 'No se pudo conectar'
                    return {'status': 'error', 'message': 'No se pudo conectar al servidor IMAP.'}

            except imaplib.IMAP4.error as e:
                logging.error(f"Error de autenticación IMAP: {e}")
                record.estado = 'Error de autenticación'
                return {'status': 'error', 'message': 'Error de autenticación. Verifica el correo y la contraseña.'}
    ...
    def obtener_oc(self):
        for record in self.env['servidor.correos'].search([('estado', '=', 'Conexión establecida')]):
            try:
                ...
                for num in mensajes[0].split():
                    ...
                    for respuesta in mensaje:
                        if isinstance(respuesta, tuple):
                            ...
                            valores_oc = ['OC', 'Order ', "ORDER", "order", "Orden", "ORDEN", "orden", 'orden de compra', 'Orden de Compra', 'Orden de compra', 'Purchase', 'PO', 'ORDEN DE COMPRA', 'oc' ]
